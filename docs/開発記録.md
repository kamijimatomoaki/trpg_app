...
## Function Call引数パース修正 (2025-09-24)

### 問題
デプロイ後、GM応答生成時にFunction Call処理でエラーが発生：
- Vertex AIのstruct形式引数を辞書変換時に失敗
- が動作しない

### 修正内容
`backend/main.py` line 1167-1174:
```python
# 修正前
...
### 結果
- Function Call引数の正しい変換を実装
- GM応答でのダイスロール機能が修復
- エラーログでstruct形式引数の確認も追加
...
デプロイ後、GM応答生成時にFunction Call処理でエラーが発生：
- Vertex AIのstruct形式引数を辞書変換時に失敗
- `dict(function_call.args)`が動作しない
...
## 画像表示機能の修正 (2025-09-24)

### 問題
ゲーム中に画像生成が行われているが、UI上に表示されない問題を発見：
1. バックエンドでは画像生成処理が行われ、imageUrlがFirestoreに保存される
2. フロントエンド（GamePlayPage.tsx）では画像URLを表示する処理がなかった
3. GM応答時の画像生成はプレースホルダーのみ使用

### 修正内容

#### フロントエンド修正
frontend/src/pages/GamePlayPage.tsx:
- ログレンダリング部分に画像表示機能を追加
- log.imageUrlが存在する場合に画像を表示
- エラーハンドリングとスタイリングも実装

#### バックエンド修正  
backend/main.py:
- GM応答時の画像生成処理を実際のImagen API呼び出しに変更
- Cloud Storage保存とエラーハンドリングを実装
- 画像生成状況の詳細ログを追加

### 結果
- GM応答時に生成された画像がゲームログ上で正しく表示されるように修正
- 画像読み込みエラー時の適切な処理も実装
- 画像生成の詳細ログでデバッグも容易に


## エピローグページプレイヤーハイライト調査 (2025-09-24)

### 調査内容
エピローグページの「ガレン 🗡️ 勇敢な冒険者 活躍度 33.3% (3/9 ターン)」などのプレイヤーハイライト部分のデータ参照の適切性を調査。

### 発見した問題

#### 1. 重大なバグ：ダイスロールデータ参照
**場所：** `backend/main.py` line 2315
```python
# 現在のコード（問題のある実装）
dice_rolls = [log for log in game_logs if log.get('type') == 'dice_roll']

# 問題点：
# - playerIdでフィルタリングしていない
# - 全プレイヤーのダイスロールが各プレイヤーに割り当てられる
# - ダイス統計が全員同一になってしまう
```

**修正が必要：**
```python
dice_rolls = [log for log in game_logs if log.get('type') == 'dice_roll' and log.get('playerId') == player_id]
```

#### 2. ハイライトモーメントの問題
**場所：** `backend/main.py` line 2317-2322
```python
if player_actions:
...
**問題点：**
- 単純に最初と最後の行動を取得するだけ
- 「印象的な行動: 終わる」など不適切な内容が表示される
- 行動の内容や重要性を評価していない

#### 3. フロントエンド統計計算への影響
**場所：** `frontend/src/components/PlayerHighlights.tsx`

**正常動作項目：**
- ✅ 行動回数：`contrib.key_actions.length`
- ✅ 活躍度計算：`(actionCount / totalTurns) * 100`

**バックエンドバグの影響を受ける項目：**
- ⚠️ ダイス回数：`contrib.dice_rolls.length` 
- ⚠️ 平均出目：ダイス出目の正規表現抽出
- ⚠️ 最高/最低出目

### 影響範囲
1. **プレイヤー統計の不正確性**：各プレイヤーが全員のダイス結果を持つため公正な評価ができない
2. **ハイライトの品質**：意味のない行動が「印象的」として表示される
3. **称号システム**：ダイス統計に基づく称号が正しく機能しない

### 修正優先度
- **最高優先**：ダイスロールのplayerIdフィルタリング修正
- **高優先**：ハイライトモーメント抽出ロジックの改善
- **中優先**：統計表示の検証とテスト

---

## JSON形式レスポンス表示問題の調査結果 (2025-09-24)

### 問題の概要
GM応答生成時に、構造化されたJSON形式の応答（`{"narration": "...", "imagePrompt": "..."}`）がそのままテキストとして表示される問題について調査しました。

### 調査結果

#### 根本原因
**Function Calling後の複雑な応答テキスト抽出処理において、JSONパースに失敗しJSON文字列がそのまま `narration` に設定されてしまう**ことが原因でした。

#### 問題箇所の特定

**1. JSONパース処理の信頼性問題**
- **場所**: `backend/main.py` lines 1460-1498, 1572
- **問題**: 複雑なフォールバック処理により、JSON解析失敗時にJSONテキストそのものを `narration` に設定してしまう
- **具体例**: `narration = response_text` でJSON文字列をそのまま代入

**2. Function Calling後の応答抽出の不安定性**
- **場所**: `backend/main.py` lines 1295-1521
- **問題**: Function Calling完了後の応答テキスト抽出が複雑すぎて、適切にテキストを取得できない場合がある
- **影響**: 「追加応答生成も失敗」時のフォールバック応答が不十分

**3. GMチャット機能は正常動作**
- **場所**: `backend/main.py` lines 2186-2274
- **状況**: 単純に `response.text` を使用しており問題なし（JSON構造化応答を期待していない）

#### 技術的詳細

**Geminiの応答処理フロー問題点:**
1. Geminiは指示通りにJSON形式で応答を返す
2. Function Calling後の応答抽出で複数の方法を試行
3. 各方法でテキスト取得に失敗すると「追加応答生成」を試行
4. それも失敗すると基本フォールバック処理に入る
5. **最終的にJSONクリーニング処理でパースに失敗**
6. **JSON文字列がそのまま `narration` に設定される**

#### 影響範囲
- **主要影響**: プレイヤーアクション後のGM応答生成
- **影響なし**: GMチャット機能（別の処理フロー）
- **副次影響**: 画像生成も適切に動作しない可能性

### 対策方針
1. **JSONパース処理の簡略化**：複雑なフォールバック処理を見直し
2. **応答抽出の安定化**：Function Calling後のテキスト抽出を単純化
3. **エラーハンドリングの改善**：JSON解析失敗時の適切なフォールバック実装
4. **デバッグ情報の充実**：問題発生時の詳細なログ出力

---

## Veo2画像生成処理の429エラー調査結果 (2025-09-24)

### 調査概要
Veo2の画像生成処理で429エラーが発生している可能性について調査を実施しました。現在のアプリケーションではVeo2は動画生成に使用されており、画像生成にはImagen 4.0が使用されていることが判明しました。

### 主要調査結果

#### 1. 画像生成の実装状況

**使用モデル**: Imagen 4.0 Fast Generate 001
- **場所**: `backend/main.py` line 373
- **初期化**: `ImageGenerationModel.from_pretrained("imagen-4.0-fast-generate-001")`

**画像生成プロンプト処理**:
- **場所**: `backend/main.py` lines 1604-1648 (GM応答用)
- **プロンプトソース**: Gemini 2.5 FlashのJSON応答内の `imagePrompt` フィールド
- **言語設定**: `language="en"` で英語プロンプトを使用

#### 2. Veo2の実際の用途

**Veo2は動画生成専用**:
- エピローグ動画生成: `backend/main.py` lines 551-608
- オープニング動画生成: `backend/main.py` lines 815-938
- モデル: `veo-2.0-generate-001` (Google GenerativeAI SDK経由)

#### 3. 現在の429エラー対策状況

**既存の対策**:
- **出力トークン制限**: `max_output_tokens=2048` (line 1144)
- **候補数制限**: `candidate_count=1` (line 1145)
- **ゲーム履歴要約**: 最大5エントリ、8000トークン制限 (line 1001)

**不足している対策**:
- プロンプト長の制限なし（現在約1600行の巨大プロンプト）
- 429エラー専用のハンドリングなし
- 英語翻訳前の要約処理なし

#### 4. プロンプト長の問題

**GM応答プロンプトの構造** (lines 1038-1107):
```text
総長: 約1600行
- ルール説明: 約30行
- 世界観: 変動（scenario dependent）
- ゲーム履歴: 最大8000トークン
- プレイヤーアクション: 変動
- 出力形式指示: 約30行
```
...
- 履歴が蓄積されると非常に長大になる
- 複雑なJSONフォーマット指示が冗長
- 英語翻訳処理は確認されなかった（画像プロンプトは既に英語で生成される）

### Web検索による429エラー情報

#### Veo2の制限状況
- **クォータ制限**: テスト段階で非常に厳しい制限（1-2動画生成後に429エラー）
- **レート制限**: プレビューモデルで特に厳しい制限
- **対処法**: 請求有効化、高いティアへのアップグレード、時間をおいた再試行

#### Google AI API共通対策
- **レート制限管理**: RPD（Requests Per Day）が太平洋標準時午前0時にリセット
- **Dynamic Shared Quota**: Gemini 2.0以降はデフォルトでDSQ使用
- **エラーハンドリング**: 429発生時の適切な待機と再試行
...
# 改善案
def create_optimized_prompt(scenario, history, actions, turn):
    # 履歴の要約をより積極的に（最大3エントリ、3000トークン）
    summarized_history = summarize_game_history(history, max_recent_entries=3, max_tokens=3000)
    
    # 簡潔なルール記述
    rules = """ダイス判定が必要な行動は roll_dice を使用。
    重要な進展で check_scenario_completion を実行。
    JSON形式で narration と imagePrompt を出力。"""
    
    return f"""TRPG GM: {scenario['title']}
{rules}

履歴: {summarized_history}
行動: {actions}
...
#### 2. 429エラー専用ハンドリング
```python
def handle_image_generation_with_retry(prompt, imagen_model):
    max_retries = 3
    base_delay = 1
    
    for attempt in range(max_retries):
        try:
            return imagen_model.generate_images(
                prompt=prompt,
                number_of_images=1,
                language="en"
            )
            ...
            if "429" in str(e) or "quota" in str(e).lower():
                if attempt < max_retries - 1:
                    ...
                    print(f"⏰ 429エラー: {delay}秒待機後リトライ ({attempt+1}/{max_retries})")
                    time.sleep(delay)
                    continue
                else:
                    print(f"❌ 429エラー: 最大リトライ回数に到達")
                    return None
            raise e
```
...
#### 3. プロンプト要約処理
```python
def create_concise_image_prompt(full_prompt):
    """画像プロンプトを要約して簡潔にする"""
    if len(full_prompt) > 200:
        # 重要なキーワードを抽出
        keywords = extract_visual_keywords(full_prompt)
        return f"TRPG scene: {', '.join(keywords[:10])}"
    return full_prompt

def extract_visual_keywords(text):
    """視覚的な要素のキーワードを抽出"""
    visual_terms = ["warrior", "door", "darkness", "forest", "castle", "sword", 
                   "magic", "dragon", "treasure", "stone", "light", "shadow"]
    found_keywords = [term for term in visual_terms if term.lower() in text.lower()]
    return found_keywords
```

### 実装優先度

1. **最高優先**: GM応答プロンプトの大幅な短縮化（現在1600行 → 目標200行以下）
2. **高優先**: 429エラー専用のリトライ機構実装
3. **中優先**: 画像プロンプトの要約処理追加
4. **低優先**: Veo2動画生成の429エラー対策（現在は画像生成が主要課題）

### 技術的詳細

**現在の画像生成フロー**:
1. Gemini 2.5 Flash → JSON応答 → imagePrompt抽出
2. Imagen 4.0 → 画像生成 → Cloud Storage保存
3. エラー時 → プレースホルダー画像使用

**問題箇所**: 主にステップ1のプロンプト長とステップ2の429エラーハンドリング不足

## 2025-09-24: Veo2動画生成の英語翻訳処理調査

### 調査結果サマリー

**Veo2動画生成での英語翻訳処理**を調査した結果、以下が判明：

1. **翻訳処理の場所**: `/home/fullm/trpg_app/backend/main.py`の以下2箇所
   - `generate_epilogue_video()` (line 610): エピローグ動画生成
   - オープニング動画生成処理 (line 926-937)

2. **現在の翻訳方式**: **Geminiを使用していない**
   - 簡単な文字列置換とハードコードされた英語テンプレートのみ
   - 日本語→英語の本格的な翻訳処理は存在しない

3. **プロンプト長の問題**: **429エラーのリスクは低い**
   - エピローグ動画: 約200文字の固定英語テンプレート
   - オープニング動画: 約170文字の固定英語テンプレート
   - どちらも十分短く、429エラーの原因ではない

4. **要約処理の実装状況**: **要約処理は未実装**
   - 現在は固定テンプレートのみ
   - 日本語プロンプトの動的な要約・翻訳機能はない

### 詳細な実装内容

#### 1. エピローグ動画生成 (line 610)
```python
...
**特徴**:
- 日本語の `player_highlights` は使用されていない
- `scenario_title` も英語プロンプトに含まれていない
- 完全に固定のテンプレート

#### 2. オープニング動画生成 (line 927)
```python
...
**特徴**:
- `english_title` を作成するが、実際のプロンプトでは使用されていない
- こちらも固定テンプレート

#### 3. 画像プロンプト簡略化処理 (line 40)
```python
...
**注意**: この処理は画像生成用であり、Veo2動画生成では使用されていない

### 改善の必要性

**現在の問題点**:
1. 日本語のシナリオ情報が動画プロンプトに反映されていない
2. プレイヤーハイライトなどの具体的な情報が活用されていない
3. 動的な翻訳・要約処理が実装されていない

**推奨する改善案**:
1. **Geminiを活用した翻訳・要約処理の実装**
   - 日本語のゲーム情報を英語に翻訳
   - 重要な要素を抽出して簡潔な動画プロンプトを生成
...
3. **コンテキストの活用**
   - シナリオタイトル、プレイヤーアクション、ゲーム結果の詳細を反映
